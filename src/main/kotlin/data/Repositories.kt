...
class ArticleRepository(
     ...
     */
    suspend fun getList(catalogId: Long, pageNumber: Long) : Result<List<ArticleDto>> {
        ...
        val response = client.get("http://localhost:port/api/chapters/$catalogId?pageNumber=$pageNumber").body<ArticleResponse?>()
        return when {
    ...
    /**
     * Refreshes the data in the database by deleting the old articles and inserting the new ones and then updates the article status
     * @param response The response from the server
     * @param catalogId The ID of the catalog
     */
    private suspend inline fun refreshData(response: ArticleResponse, catalogId: Long) = coroutineScope {
