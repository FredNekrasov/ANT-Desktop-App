...
fun Sacraments(state: MainArticleState) {
    ...
    Column(Modifier.fillMaxWidth().padding(8.dp), Arrangement.Center) {
        state.list.fastForEach {
            ...
            FredTitle(text = it.title, textUnit = 26.sp)
            Spacer(modifier = Modifier.height(8.dp))
            FredText(text = it.description, modifier = Modifier.fillMaxWidth(), textAlign = TextAlign.Center, textUnit = 24.sp)
        }
        Spacer(modifier = Modifier.height(8.dp))
        HorizontalDivider()
        Spacer(modifier = Modifier.height(8.dp))
